...
def process_date(api, symbol, prev_date, curr_date, etoro_sl, max_sl, max_tp, exit_timesteps, no_trade_min, no_trade_max):
    ...
    if exit_time is None:
        if len(data) == 390:
            exit_timestep = -exit_timesteps-1
        else:
            exit_timestep = -exit_timesteps - 1 + (390 - len(data))
...
def simulation(api, calendar, symbol, etoro_sl, max_sl, max_tp, close_trade, no_trade_min, no_trade_max, pool):
    ...
    for date_i in range(1, len(calendar)):
        ...
        result = process_date(api, symbol, prev_date, curr_date, etoro_sl, max_sl, max_tp, close_trade, no_trade_min, no_trade_max)
...
if __name__ == "__main__":
    ...
    symbol = sys.argv[1] #"QQQ"
    max_sl = int(sys.argv[2]) #100
    max_tp = int(sys.argv[3]) #100
    close_trade = int(sys.argv[4]) #1
    no_trade_min = float(sys.argv[5]) #0.1
    no_trade_max = float(sys.argv[6]) #2
    
    calendar = api.get_calendar(start=start_date, end=end_date)
    
    pool = mp.Pool(10)
    open_trades, close_trades = simulation(api, calendar, symbol, etoro_sl, max_sl, max_tp, close_trade, no_trade_min, no_trade_max, pool)
    pool.close()
