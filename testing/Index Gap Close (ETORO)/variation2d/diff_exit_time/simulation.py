...
def process_date(api, symbol, ma_data, prev_date, curr_date, etoro_sl, exit_timesteps, no_trade_min, no_trade_max, tp_limit, tp_perc, sl_perc):
    ...
    if prev_ma > curr_ma and day_open > prev_close:
        ...
        entry_type = "bid"
        direction = "SHORT"
    elif prev_ma < curr_ma and day_open < prev_close:
        etoro_sl = day_open - day_open * etoro_sl/100
        if gap <= tp_limit:
            tp = day_open + abs(day_open - prev_close)
            gap_sl = day_open - abs(day_open - prev_close) * sl_perc/100
    ...
    if len(data) == 390:
        exit_timestep = -exit_timesteps
    else:
        exit_timestep = -exit_timesteps + (390 - len(data))
    
    if exit_time is None:
        if day_open > prev_close:
            exit_time = period_timestamps[exit_timestep]
            exit_type = "ask"
...
def simulation(api, calendar, ma_data, symbol, etoro_sl, close_trade, no_trade_min, no_trade_max, tp_limit, tp_perc, sl_perc, pool):
    ...
    for date_i in range(1, len(calendar)):
        ...
        result = process_date(api, symbol, ma_data, prev_date, curr_date, etoro_sl, close_trade, no_trade_min, no_trade_max, tp_limit, tp_perc, sl_perc)
...
if __name__ == "__main__":
    ...
    symbol = sys.argv[1]
    close_trade = int(sys.argv[2])
    no_trade_min = float(sys.argv[3])
    no_trade_max = float(sys.argv[4])
    tp_limit = float(sys.argv[5])
    tp_perc = int(sys.argv[6])
    sl_perc = int(sys.argv[7])
    ma_length = int(sys.argv[8])
    ...
    pool = mp.Pool(10)
    open_trades, close_trades = simulation(api, calendar, ma_data, symbol, etoro_sl, close_trade, no_trade_min, no_trade_max, tp_limit, tp_perc, sl_perc, pool)
    pool.close()

    log_trades(open_trades, close_trades, name=str(close_trade), save=True)
