...
def get_ma(api, symbol, start_date, end_date):
    
    start_date_str = str(start_date.date).split(" ")[0]
    end_date_str = str(end_date.date).split(" ")[0]
    
    bar_data = api.get_bars(symbol, TimeFrame.Day, start_date_str, end_date_str, adjustment="all").df
    bar_data = bar_data.tz_convert('US/Eastern')
    
    return bar_data["close"].mean()
    
def process_date(api, symbol, calendar, date_num, etoro_sl, exit_time, no_trade_min, tp_limit, tp_perc, sl_perc, ma_length):
    ...
    prev_ma = get_ma(api, symbol, calendar[date_num-1-ma_length], calendar[date_num-2])
    curr_ma = get_ma(api, symbol, calendar[date_num-ma_length], calendar[date_num-1])
    
    if day_open > prev_close and prev_ma > curr_ma:
    ...
    elif day_open < prev_close and prev_ma < curr_ma:
    ...
    for data_i in range(len(data) - exit_time):
        ...
        if day_open > prev_close:
            if period_high > sl:
                exit_time, exit_price = get_quotes_exit(api, symbol, period_timestamp, sl, 1, exit_time)
                exit_price_present = True
                break
            if period_low < tp:
                exit_time, exit_price = get_quotes_exit(api, symbol, period_timestamp, tp, 0, exit_time)
                exit_price_present = True
                break
            
        elif day_open < prev_close:
            if period_high > tp:
                exit_time, exit_price = get_quotes_exit(api, symbol, period_timestamp, tp, 1, exit_time)
                exit_price_present = True
                break
            if period_low < sl:
                exit_time, exit_price = get_quotes_exit(api, symbol, period_timestamp, sl, 0, exit_time)
                exit_price_present = True
                break
    
    if exit_price_present == False:
        if day_open > prev_close:
            exit_time, exit_price = get_quotes_entry(api, symbol, data.index.values[-1], 0, exit_time)
        elif day_open < prev_close:
            exit_time, exit_price = get_quotes_entry(api, symbol, data.index.values[-1], 1, exit_time)
...
if __name__ == "__main__":
    ...
    symbol = sys.argv[1]
    exit_time = int(sys.argv[2])
    no_trade_min = float(sys.argv[3])
    tp_limit = float(sys.argv[4])
    tp_perc = float(sys.argv[5])
    sl_perc = float(sys.argv[6])
    ...
    for date_i in range(ma_length+1, len(calendar)):
        
        print(symbol, str(calendar[date_i].date).split(" ")[0])
        
        trade_data = process_date(api, symbol, calendar, date_i, etoro_sl, exit_time, no_trade_min, tp_limit, tp_perc, sl_perc, ma_length)
        all_trade_data.append(trade_data)
            
    all_trade_data = pd.concat(all_trade_data)
    all_trade_data.to_csv(symbol + "_" + str(exit_time) + "_" + str(no_trade_min) + "_" + str(tp_limit) + "_" + str(tp_perc) + "_" + str(sl_perc) + "_" + str(ma_length) + ".csv")
