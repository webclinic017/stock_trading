...
def get_minute_data(api, symbol, date):
...
def process_date(api, symbol, calendar, date_num, etoro_sl, max_sl, max_tp, exit_timesteps, no_trade_min, no_trade_max, ma_length):
    
    prev_date = calendar[date_i-1]
    curr_date = calendar[date_i]        
        
    data = get_minute_data(api, symbol, prev_date)
    prev_close = data["close"][-1]
    
    data = get_minute_data(api, symbol, curr_date)
...
if __name__ == "__main__":
    ...
    for date_i in range(ma_length+1, len(calendar)):
        ...
        trade_data = process_date(api, symbol, calendar, date_i, etoro_sl, max_sl, max_tp, exit_time, no_trade_min, no_trade_max, ma_length)
        all_trade_data.append(trade_data)
            
    all_trade_data = pd.concat(all_trade_data)
    all_trade_data.to_csv(symbol + "_" + str(max_sl) + "_" + str(max_tp) + "_" + str(exit_time) + "_" + str(no_trade_min) + "_" + str(no_trade_max) + "_" + str(ma_length) + ".csv")
