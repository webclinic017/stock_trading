...
def simulation(api, symbol, prices, shadow_length, pool):
	...
	for row in range(len(prices)-1):
		...
		if lower_wick_length > upper_wick_length and lower_wick_length > body_length * shadow_length:
			if len(open_trades_times) == len(close_trades_times):
				open_trades_times.append([indexes[row+1], close_price])

		if upper_wick_length > lower_wick_length and upper_wick_length > body_length * shadow_length:
...
if __name__ == "__main__":
	...
	pool = mp.Pool(10)
	open_trades, close_trades = simulation(api, symbol, prices, shadow_length, pool)
	pool.close()
