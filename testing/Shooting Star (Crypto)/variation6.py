...
def simulation(prices, shadow_length):
    ...
    for row in range(len(prices)-1):
        ...
        if prices.index.year.values[row] == 2018 and prices.index.month.values[row] == 2 and (prices.index.day.values[row] in [2, 3, 4, 5] or (prices.index.day.values[row] == 2 and prices.index.hour.values[row] >= 17)):
            continue
        ...
        if lower_wick_length > upper_wick_length and lower_wick_length * shadow_length > body_length:
