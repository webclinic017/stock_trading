...
def simulation(api, symbol, prices, shadow_length, short_ma, long_ma, pool):
	...
	for row in range(len(prices)-1):
		...
		if close_price > open_price and close_price == high_price and lower_wick_length > body_length * shadow_length:
			if len(open_trades_times) == len(close_trades_times):
				open_trades_times.append([indexes[row+1], close_price])

		if close_price < open_price and close_price == low_price and upper_wick_length > body_length * shadow_length and short_ma_price < long_ma_price:
...
if __name__ == "__main__":
	...
	end_date = "2018-07-01"
	period = 15
	shadow_length = float(sys.argv[1])
	short_ma = 8
	long_ma = 13
	...
	prices = get_crypto_prices(api, symbol, start_date, end_date, TimeFrameUnit.Minute, [period])[period]
	...
	log_trades(open_trades, close_trades, name=sys.argv[1], save=True)
